...
// New creates a parser and primes current/peek token state.
...
// Errors returns parser errors encountered while parsing.
func (p *Parser) Errors() []string {
	return p.errors
}

// ParseProgram parses a full token stream into a program AST.
...
func (p *Parser) parseStatement() ast.Statement {
	...
	case token.FUNCTION:
		if p.curToken.Literal == "function" && p.peekTokenIs(token.IDENT) {
...
func (p *Parser) parseFunctionDefinitionStatement() ast.Statement {
	...
	function := &ast.FunctionLiteral{Token: functionToken}
...
func (p *Parser) parseIfExpression() ast.Expression {
	...
	expression.Branches = append(expression.Branches, ast.IfBranch{Condition: condition, Consequence: consequence})
	...
	for p.peekTokenIs(token.ELSEIF) {
		...
		p.nextToken()
		branchCondition := p.parseExpression(LOWEST)
		if !p.expectPeek(token.RPAREN) {
			return nil
		}
		...
		branchConsequence := p.parseBlockStatement()
		expression.Branches = append(expression.Branches, ast.IfBranch{Condition: branchCondition, Consequence: branchConsequence})
	}
...
func (p *Parser) parseForExpression() ast.Expression {
	...
	if !p.curTokenIs(token.SEMICOLON) {
		expression.Init = p.parseForClauseStatement()
	}
	...
	if !p.curTokenIs(token.RPAREN) {
		expression.Post = p.parseForClauseStatement()
	}
